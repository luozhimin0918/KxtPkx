...
import com.library.util.ShellUtils.CommandResult;
 ...
 * 如：install(Context, String) 安装应用，如果是系统应用或已经root，则静默安装，
 * 否则一般安装uninstall(Context, String) 卸载应用，如果是系统应用或已经root，则静默卸载，
 * 否则一般卸载isSystemApplication(Context, String) 判断应用是否为系统应用
 ...
 * <li>{@link PackageUtils#isTopActivity(Context, String)} whether the app whost package's name
 * is packageName is on the
 * top of the stack</li>
...
public class PackageUtils
{
    ...
    public static int installSilent(Context context, String filePath, String pmParams)
    {
        ...
        StringBuilder command = new StringBuilder().append
                ...
                .append(pmParams == null ? "" : pmParams).append(" ").append(filePath.replace(" " +
                        "", "\\ "));
        CommandResult commandResult = ShellUtils.execCommand(command.toString(),
        ...
        LogUtil.e(TAG,
                new StringBuilder().append("installSilent successMsg:").append(commandResult
                        .successMsg)
    ...
    public static int uninstallSilent(Context context, String packageName, boolean isKeepData)
    {
        ...
        LogUtil.e(TAG,
                new StringBuilder().append("uninstallSilent successMsg:").append(commandResult
                        .successMsg)
                        .append(", ErrorMsg:").append(commandResult.errorMsg).toString());
        if (commandResult.errorMsg == null)
        {
     ...
     * @param context
     * @param packageName
     * @return if params error or task stack is null, return null, otherwise retun whether the
     * app is on the top of
     * stack
     */
    ...
    public static int getInstallLocation()
    {
        ...
        if (commandResult.result == 0 && commandResult.successMsg != null && commandResult
                .successMsg.length() > 0)
        {
            try
            {
            ...
            {
                e.printStackTrace();
                LogUtil.e(TAG, "pm get-install-location error");
            }
        }
    ...
    /**
     * Installation return code<br/>
     * the new package is requested a shared user which is already installed on the device and
    ...
    /**
     * Installation return code<br/>
     * the new package failed because it has specified that it is a test-only package and the
    ...
    /**
     * Installation return code<br/>
     * the new package couldn't be installed in the specified install location because the media
     * is not available.
     */
